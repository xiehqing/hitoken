...
// HiTokenContext Hi-Token context for current request | Hi-Token上下文，用于当前请求
type HiTokenContext struct {
	ctx     adapter.RequestContext
	manager *manager.Manager
}

// NewContext creates a new Hi-Token context | 创建新的Hi-Token上下文
func NewContext(ctx adapter.RequestContext, mgr *manager.Manager) *HiTokenContext {
	return &HiTokenContext{
		ctx:     ctx,
		manager: mgr,
	}
}
...
func (c *HiTokenContext) GetTokenValue() string {
...
// IsLogin 检查当前请求是否已登录
func (c *HiTokenContext) IsLogin() bool {
	token := c.GetTokenValue()
	return c.manager.IsLogin(token)
}

// CheckLogin 检查登录（未登录抛出错误）
func (c *HiTokenContext) CheckLogin() error {
	token := c.GetTokenValue()
	return c.manager.CheckLogin(token)
}

// GetLoginID 获取当前登录ID
func (c *HiTokenContext) GetLoginID() (string, error) {
	token := c.GetTokenValue()
	return c.manager.GetLoginID(token)
}

// HasPermission 检查是否有指定权限
func (c *HiTokenContext) HasPermission(permission string) bool {
...
// HasRole 检查是否有指定角色
func (c *HiTokenContext) HasRole(role string) bool {
	loginID, err := c.GetLoginID()
	if err != nil {
		return false
	}
...
func (c *HiTokenContext) GetRequestContext() adapter.RequestContext {
	return c.ctx
}

// GetManager 获取管理器
func (c *HiTokenContext) GetManager() *manager.Manager {
	return c.manager
}
