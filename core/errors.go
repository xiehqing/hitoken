...
// HiTokenError Represents a custom error with error code and context | 自定义错误类型，包含错误码和上下文信息
type HiTokenError struct {
...
func (e *HiTokenError) Error() string {
	if e.Err != nil {
...
func (e *HiTokenError) Unwrap() error {
	return e.Err
}

// WithContext Adds context information to the error | 为错误添加上下文信息
func (e *HiTokenError) WithContext(key string, value any) *HiTokenError {
	if e.Context == nil {
...
func (e *HiTokenError) GetContext(key string) (any, bool) {
	if e.Context == nil {
		return nil, false
	}
...
func (e *HiTokenError) Is(target error) bool {
	t, ok := target.(*HiTokenError)
	if !ok {
		return false
	}
...
// NewError Creates a new Hi-Token error | 创建新的 Hi-Token 错误
func NewError(code int, message string, err error) *HiTokenError {
	return &HiTokenError{
		Code:    code,
		Message: message,
		Err:     err,
...
// NewErrorWithContext Creates a new Hi-Token error with context | 创建带上下文的 Hi-Token 错误
func NewErrorWithContext(code int, message string, err error, context map[string]any) *HiTokenError {
	return &HiTokenError{
		Code:    code,
		Message: message,
		Err:     err,
...
func NewNotLoginError() *HiTokenError {
...
func NewPermissionDeniedError(permission string) *HiTokenError {
...
func NewRoleDeniedError(role string) *HiTokenError {
...
func NewAccountDisabledError(loginID string) *HiTokenError {
...
func NewPathAuthRequiredError(path string) *HiTokenError {
...
func NewPathNotAllowedError(path string) *HiTokenError {
...
// GetErrorCode Extracts error code from HiTokenError | 从HiTokenError中提取错误码
func GetErrorCode(err error) int {
	var saErr *HiTokenError
...
const (
	...
	// Hi-Token specific error codes (10000-19999) | Hi-Token 特定错误码 (10000-19999)
