...
// setupTestRouter 创建测试路由器和初始化 Hi-Token
...
func mockLoginWithRole(loginID interface{}, roles []string) string {
	token, _ := htputil.Login(loginID)
	htputil.SetRoles(loginID, roles)
	return token
}
...
func mockLoginWithPermission(loginID interface{}, permissions []string) string {
	token, _ := htputil.Login(loginID)
	htputil.SetPermissions(loginID, permissions)
	return token
}
...
func TestCheckDisable_IsDisabled(t *testing.T) {
	...
	htputil.Disable(loginID, 3600) // 封禁 1 小时
